...
from test_ohol import FindOHOLStocks, LoadPastData, MIS, PastData, TODAY
...
if __name__ == '__main__':
    ...
    if not DRY_RUN:
	order_info_file = "/home/somasm/GITRepo/PythonTrade/BETA/OrderInfo/%s.orders" % TODAY
	...
	if AutoFetch == True:
		...
		try:
		    ...
		    if session_token is not None or session_token != '':
			break;
		    else:
			try_count+=1
		except:
	...
	if PARALLEL:
	    ...
	    if(len(BuySyms.keys())>0):
		BuyOrders = {}
		for bsym,bprice in BuySyms.items():
		    PlaceOrder('Buy', bsym, bprice, BuyOrders)

	    if(len(SellSyms.keys())>0):
		SellOrders = {}
		for ssym,sprice in SellSyms.items():
